...
class StockSync {
	 ...
	 *
	 * Only the level is written. A product's status is never touched here: this sync
	 * does not decide whether an article belongs in the shop, so a product a person
	 * or the product sync drafted keeps its level updated behind the scenes and stays
	 * drafted.
	 *
	 ...
	 * nothing in this job walks the catalogue looking for what the feed left out, so
	 * the run is over as soon as the levels it did carry have been written.
	 *
	 * @param int $run Run identifier.
	 * @return void
	 */
