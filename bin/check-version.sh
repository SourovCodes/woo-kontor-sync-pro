...
# the WKSYNC_VERSION constant — and, at release time, in the git tag as well. A build
...
MAIN_FILE="$PLUGIN_DIR/woo-kontor-sync-pro.php"
EXPECTED="${1:-}"
...
CONSTANT_VERSION="$(sed -n "s/^define( 'WKSYNC_VERSION', '\([^']*\)' );.*/\1/p" "$MAIN_FILE" | head -n 1)"

[ -n "$HEADER_VERSION" ] || die "No 'Version:' header found in $MAIN_FILE."
[ -n "$CONSTANT_VERSION" ] || die "No WKSYNC_VERSION constant found in $MAIN_FILE."

if [ "$HEADER_VERSION" != "$CONSTANT_VERSION" ]; then
	die "Version header ($HEADER_VERSION) and WKSYNC_VERSION ($CONSTANT_VERSION) disagree."
fi
