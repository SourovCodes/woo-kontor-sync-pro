#!/usr/bin/env bash
#
# Check that languages/woo-kontor-sync-pro.pot still describes the source.
#
...
WORK="$(mktemp -d "${TMPDIR:-/tmp}/wksync-i18n.XXXXXX")"
trap 'rm -rf "$WORK"' EXIT

wp i18n make-pot "$PLUGIN_DIR" "$WORK/$DOMAIN.pot" \
	--slug="$DOMAIN" \
	--domain="$DOMAIN" \
	--package-name="Woo Kontor Sync Pro" \
...
$committed = wksync_msgids( $argv[1] );
$extracted = wksync_msgids( $argv[2] );
